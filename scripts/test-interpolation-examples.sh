...
run_test "Basic interpolation with defaults" \
    "./drun -f examples/03-interpolation.drun greet name=Andy" \
    "Hello, friend Andy!" \
...
run_test "Custom parameter values" \
    "./drun -f examples/03-interpolation.drun greet name=Bob title=buddy" \
    "Hello, buddy Bob!" \
...
run_test "Deploy task with constraints" \
    "./drun -f examples/03-interpolation.drun deploy environment=staging app_version=v1.2.3" \
...
run_test "Deploy with default version" \
    "./drun -f examples/03-interpolation.drun deploy environment=dev" \
...
run_test "Backup with default name" \
    "./drun -f examples/03-interpolation.drun backup source_path=/home/user/data" \
...
run_test "Backup with custom name" \
    "./drun -f examples/03-interpolation.drun backup source_path=/tmp/data backup_name=my-backup" \
...
run_test "Empty parameter values" \
    "./drun -f /tmp/test-empty.drun empty name=" \
    "Name: '', Title: ''"
...
run_test "Special characters in parameters" \
    "./drun -f /tmp/test-special.drun special \"message=Hello World!\"" \
    "Message: Hello World!"
...
run_test "Multiple same variable" \
    "./drun -f /tmp/test-repeat.drun repeat word=echo" \
    "echo echo echo!"
...
run_test "Undefined variables remain as placeholders" \
    "./drun -f /tmp/test-undefined.drun undefined name=Alice" \
