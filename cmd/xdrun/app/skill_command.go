...
func buildDrunBasicsGuide(projectName string) string {
	return fmt.Sprintf(strings.Join([]string{
		...
		"6. When adding hard dependencies, declare them with `requires tools:`.",
		"7. Prefer small, readable tasks that explain intent with `means`, `info`, and `step`.",
		"8. For AI-driven CI or noisy checks, prefer tasks declared with `mode \"ci\"` so successful shell stdout/stderr stays buffered and only failure output is emitted.",
		"9. After editing a spec, run the narrowest relevant `xdrun` command to verify behavior.",
		"",
		"## Tool Checks",
		"",
		...
		"- If two platforms need the same user-facing workflow name, use duplicate task names only with disjoint `@platform(...)` annotations so `xdrun <task>` resolves the correct variant automatically.",
