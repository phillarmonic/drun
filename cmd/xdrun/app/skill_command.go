...
func buildDrunBasicsGuide(projectName string) string {
	return fmt.Sprintf(strings.Join([]string{
		...
		"4. When adding hard dependencies, declare them with `requires tools:`.",
		"5. Prefer small, readable tasks that explain intent with `means`, `info`, and `step`.",
		"6. For AI-driven CI or noisy checks, prefer tasks declared with `mode \"ci\"` so successful shell stdout/stderr stays buffered and only failure output is emitted.",
		"7. After editing a spec, run the narrowest relevant `xdrun` command to verify behavior.",
		"",
		"## Tool Checks",
		"",
